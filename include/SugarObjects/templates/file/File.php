...
class File extends Basic{
	...
	function save($check_notify=false){
 		...
 		if (isset($_FILES['uploadfile']) && $upload_file->confirm_upload())
		{
			...
			$move=true;
		} else  if ( ! empty($_REQUEST['duplicateId'])) {
			//Duplicate the file on the file system
			$class = get_class($this);
			$oldRecord = new $class();
			$oldRecord->retrieve($_REQUEST['duplicateId']);
		   	$this->filename = $oldRecord->filename;
    		$this->file_mime_type = $oldRecord->file_mime_type;
			$this->file_ext = $oldRecord->file_ext;
		}
		...
		parent::save($check_notify);
 		if ($move) {
			...
			//Duplicate the file on the file system
		   	$upload_file->duplicate_file($_REQUEST['duplicateId'], $this->id, $oldRecord->filename);
		}
 		return $this->id;
