...
class OutboundEmail {
		...
		if(!empty($system->id) ) 
		{
			if ($system->mail_sendtype == 'SMTP') 
			{
			    ...
			    if( !empty($system->mail_smtpserver) )
				    $ret[] = array('id' =>$system->id, 'name' => "$system->name", 'mail_smtpserver' => $system->mail_smtpserver,
	...
	{
	    $updateFields = array('mail_smtpserver', 'mail_smtpport','mail_smtpauth_req','mail_smtpssl');
