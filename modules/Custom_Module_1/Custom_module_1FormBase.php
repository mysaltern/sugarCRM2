...
class Custome_module_1FormBase {
	...
	function checkForDuplicates($prefix) {
		...
		$query = '';
		$baseQuery = 'select id, name, sales_stage,amount, date_closed  from custom_module_1 where deleted!=1 and (';
	...
	function buildTableForm($rows, $mod = 'Custom') {
		if (!empty($mod)) {
		...
		$form .= "<tr class='pagination'><td colspan='$cols'><table width='100%' cellspacing='0' cellpadding='0' border='0'><tr><td><input type='submit' class='button' name='ContinueCustom' value='${mod_strings['LNK_NEW_OPPORTUNITY']}'></td></tr></table></td></tr><tr>";
		...
		foreach ($rows as $row) {
			...
			$form .= "<td width='1%' nowrap='nowrap'><a href='#' onclick='document.dupOpps.selectedCustom.value=\"${row['id']}\";document.dupOpps.submit();'>[${app_strings['LBL_SELECT_BUTTON_LABEL']}]</a>&nbsp;&nbsp;</td>";
			$wasSet = false;
			foreach ($row as $key => $value) {
				if ($key != 'id') {
					if (!$wasSet) {
						$form .= "<td scope='row'><a target='_blank' href='index.php?module=Custom_Module_1&action=DetailView&record=${row['id']}'>$value</a></td>";
						$wasSet = true;
					} else {
						$form .= "<td><a target='_blank' href='index.php?module=Custom_Module_1&action=DetailView&record=${row['id']}'>$value</a></td>";
					}
				}
			}
		...
		$form .= "<tr class='pagination'><td colspan='$cols'><table width='100%' cellspacing='0' cellpadding='0' border='0'><tr><td><input type='submit' class='button' name='ContinueCustom_Module_1' value='${mod_strings['LNK_NEW_CUSTOM_MODULE_1']}'></td></tr></table></td></tr><tr>";
		$form .= "</table><BR></form>";

		return $form;
	}

	function getForm($prefix, $mod = 'Custom') {
		if (!ACLController::checkAccess('Custom_Module_1', 'edit', true)) {
			return '';
		}
		if (!empty($mod)) {
			global $current_language;
		...
		<form name="{$prefix}OppSave" onSubmit="return check_form('{$prefix}OppSave')" method="POST" action="index.php">
			<input type="hidden" name="{$prefix}module" value="Custom_Module_1">
...
//	}
	...
	function getWideFormBody($prefix, $mod = 'Custom', $formname = '', $lead = '', $showaccount = true) {
		if (!ACLController::checkAccess('Custom', 'edit', true)) {
			return '';
		}
...
// end getWideFormBody
	...
	function handleSave($prefix, $redirect = true, $useRequired = false) {
		...
		if (!empty($_POST['duplicate_parent_id'])) {
			clone_relationship($focus->db, array('custom_module_1_contacts'), 'custom_id', $_POST['duplicate_parent_id'], $focus->id);
		}
		$return_id = $focus->id;

		$GLOBALS['log']->debug("Saved record with id of " . $return_id);
		if ($redirect) {
			handleRedirect($return_id, "Custom_Module_1");
		} else {
			return $focus;
		}
	}
...
// end getWideFormBody
	...
	function getFormBody($prefix, $mod = 'Custom_Module_1', $formname = '') {
		if (!ACLController::checkAccess('Custom_Module_1', 'edit', true)) {
			return '';
		}
		if (!empty($mod)) {
...
EOQ;
		...
		$javascript = new javascript();
		$javascript->setFormName($formname);
		$javascript->setSugarBean(new Custom_Module_1());
		$javascript->addRequiredFields($prefix);
		$the_form .= $javascript->getScript();
